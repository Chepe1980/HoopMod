...
def plot_well_logs(well_log, selected_depth):
    ...
    fig = make_subplots(rows=1, cols=4, subplot_titles=(
        'Vertical Stress (Sv)', 'Minimum Horizontal Stress (Shmin)',
    ...
    titles = ['Sv (MPa)', 'Shmin (MPa)', 'Shmax (MPa)', 'PP (MPa)']
    ...
    for i, (log, title, color) in enumerate(zip(logs, titles, colors), 1):
        fig.add_trace(
            go.Scatter(
                ...
                name=title,
                hovertemplate="Depth: %{y:.1f}m<br>Value: %{x:.1f}MPa<extra></extra>"
            ),
            row=1, col=i
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                showlegend=False,
                hovertemplate=f"Selected Depth: {selected_depth:.1f}m<br>Value: {selected_values[log]:.1f}MPa<extra></extra>"
            ),
            row=1, col=i
        )
...
def update_plots(Sv, Shmin, Shmax, PP, wellbore_pressure, azimuth, dip, deviation, selected_cmap):
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            hovertemplate="Angle: %{theta:.1f}°<br>Hoop Stress: %{r:.1f}MPa<extra></extra>"
        ),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatterpolar(
            ...
            hovertemplate="Angle: %{theta:.1f}°<br>Radial Stress: %{r:.1f}MPa<extra></extra>"
        ),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatterpolar(
            ...
            hovertemplate="Angle: %{theta:.1f}°<br>Shear Stress: %{r:.1f}MPa<extra></extra>"
        ),
        row=1, col=1
    )
    fig.update_polars(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='lines',
            line=dict(color='red', width=2),
            hovertemplate="Angle: %{x:.1f}°<br>Hoop Stress: %{y:.1f}MPa<extra></extra>",
            showlegend=False
        ),
        row=1, col=2
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name='Radial Stress',
            line=dict(color='blue', width=2),
            hovertemplate="Angle: %{x:.1f}°<br>Radial Stress: %{y:.1f}MPa<extra></extra>"
        ),
        row=1, col=3
    )
    fig.add_trace(
        go.Scatter(
            ...
            name='Shear Stress',
            line=dict(color='green', width=2),
            hovertemplate="Angle: %{x:.1f}°<br>Shear Stress: %{y:.1f}MPa<extra></extra>"
        ),
        row=1, col=3
    )
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            hovertemplate="X: %{x:.2f}m<br>Y: %{y:.2f}m<br>Stress: %{z:.1f}MPa<extra></extra>"
        ),
        row=2, col=1
    )
    ...
    # Add arrow showing Shmin direction
    arrow_length = 2.5
    arrow_x = arrow_length * np.cos(np.radians(azimuth + 90))
    arrow_y = arrow_length * np.sin(np.radians(azimuth + 90))
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="Angle: %{x:.1f}°<br>Magnitude: %{y:.1f}MPa<extra></extra>"
        ),
        row=2, col=3
    )
    fig.add_trace(
    ...
    fig.update_yaxes(title_text="Hoop Stress (MPa)", row=1, col=2)
    fig.update_xaxes(title_text="Angle (degrees)", row=1, col=3)
    fig.update_yaxes(title_text="Stress (MPa)", row=1, col=3)
    ...
    fig.update_yaxes(title_text="Stress (MPa)", row=2, col=3)
